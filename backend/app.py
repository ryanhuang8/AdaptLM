...
def process_query():
    ...
    try:
        ...
        # NOTE: THIS IS THE UNIVERSAL PREVIOUS OUTPUT AND PROMPT
        previous_prompt = ""
        previous_output = ""
        vector_store = PineconeVectorStore(index_name=f"user_{uid}")

        response = llm_response(chosen_llm, user_prompt, context, previous_prompt, previous_output, vector_store)
